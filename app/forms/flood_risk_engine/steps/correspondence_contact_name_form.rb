...
module FloodRiskEngine
  module Steps
    class CorrespondenceContactNameForm < BaseForm
      ...
      def self.locale_key
        "flood_risk_engine.enrollments.steps.correspondence_contact"
      end

      def self.name_max_length
        @name_max_length ||= NameFormatValidator.find_max_column_length(FloodRiskEngine::Contact, "full_name", 255)
      end

      property :full_name

      validates :full_name, presence: { message: I18n.t("#{CorrespondenceContactNameForm.locale_key}.errors.full_name.blank") }
