  ...
  RSpec.describe CompanyAddressManualForm, type: :model do
    ...
    describe "#submit" do
      ...
      context "when the form is valid" do
        ...
        let(:transient_registration) { company_address_manual_form.transient_registration }

        it "updates the transient registration with the submitted address data" do
          # Ensure the test data is properly configured:
          expect(transient_registration.company_address).to be_nil

          company_address_manual_form.submit(valid_params)

          submitted_address = transient_registration.reload.company_address
          address_data[:company_address].each do |key, value|
        ...
        context "when the address data includes extraneous white space" do
          it "strips the extraneous white space from the submitted address data" do
            # Ensure the test data is properly configured:
            address_data[:company_address].each do |key, value|
              expect(white_space_params[:company_address][key]).not_to eq(value)
              expect(white_space_params[:company_address][key].strip).to eq(value)
            end
